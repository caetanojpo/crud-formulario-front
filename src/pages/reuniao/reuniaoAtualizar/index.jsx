...
export default function ReuniaoAtualizar() {
  ...
  const enviarDados = async (e) => {
    ...
    if (dadosReuniao.categoriaId) {
      dadosReuniao.categoriaId = parseInt(dadosReuniao.categoriaId);
    }

    await Api.putRequest(url.pathname, dadosReuniao);

    if (dadosParticipante) {
      const dadosAtualizarParticipantes = dadosParticipante.map((item) => {
        return { reuniaoId, participanteId: item.value };
      });

      await Api.putRequest(
        `/participanteReuniao/${reuniaoId}`,
        dadosAtualizarParticipantes
      );
    }
            ...
            >
              ...
              >
                ...
                <ChakraSelect
                  ...
                  name="categoriaId"
                  onChange={atualizarDadosReuniao}
                  defaultValue={reuniao.categoriaId}
                >
                  {categorias.map((item) => (
